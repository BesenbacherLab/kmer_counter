...
WrongNumberOfInputException = \
    Exception('Exactly one of the input options --wig, --bed, --pos or --all_autosomes should be used')
...
def get_possible_indel_pos(chrom, pos, ref, alt, tb, buffer=50):
    ...
    if len(ref) > len(alt): # This variant is a deletion
        assert(len(alt)==1)
        assert(ref[0] == alt)
        long_seq = tb.sequence(chrom, pos, pos+buffer)
        ...
        long_seq =  alt + short_seq[1:] # Alternative seqeunce
...
def test_indels():
    ...
    print(get_possible_indel_pos("chr1", 1012306, "C", "CTC", tb ,1))
    # [(1012306, 'CT'), (1012307, 'TC'), (1012308, 'CT')]
    kmers = []
    for rpos, ralt in get_possible_indel_pos("chr1", 1012306, "C", "CTC", tb ,1): 
    ...
    print(get_possible_indel_pos("chr1", 40884142, "TCG", "T", tb, 1))
    for rpos, ralt in get_possible_indel_pos("chr1", 40884142, "TCG", "T", tb, 1): 
        c1, c2 = get_indel_contexts("chr1", rpos, 2, tb)
        kmers.append(c1)
    print(kmers)
